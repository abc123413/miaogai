...
type ChartMode = 'distribution' | ModelAnalyticsChartTab
...
interface ConsumptionDistributionChartProps {
  ...
  defaultAnalyticsTab?: ModelAnalyticsChartTab
}
...
const MODE_TITLE_KEYS: Record<ChartMode, string> = {
  distribution: 'Quota Distribution',
  trend: 'Call Trend',
  proportion: 'Call Count Distribution',
  top: 'Call Count Ranking',
}
...
export function ConsumptionDistributionChart(
  ...
  const spec = useMemo(() => {
    const specMap: Record<ChartMode, () => unknown> = {
      distribution: () =>
        chartType === 'bar' ? chartData.spec_line : chartData.spec_area,
      trend: () => chartData.spec_model_line,
      proportion: () => chartData.spec_pie,
      top: () => chartData.spec_rank_bar,
    }
    return specMap[chartMode]()
  ...
  const specType = typeof (spec as Record<string, unknown>)?.type === 'string'
    ...
    <div className='overflow-hidden rounded-lg border'>
      <div className='flex w-full flex-col gap-1.5 border-b px-3 py-2 sm:gap-3 sm:px-5 sm:py-3 lg:flex-row lg:items-center lg:justify-between'>
        ...
        <div className='flex flex-wrap items-center gap-2'>
          {/* Mode selector: Distribution + Analytics tabs */}
          <div className='bg-muted/60 inline-flex h-7 overflow-x-auto rounded-lg border p-0.5 sm:h-8'>
            {MODEL_ANALYTICS_CHART_OPTIONS.map((tab) => (
              <button
                key={tab.value}
                type='button'
                onClick={() => setChartMode(tab.value)}
          ...
          {/* Chart type icons (always clickable, switches to distribution mode) */}
                ...
                return (
                  <button
                    ...
                    className={`inline-flex shrink-0 items-center gap-1.5 rounded-md px-3 text-xs font-medium transition-colors ${
                      chartMode === 'distribution' && chartType === item.value
                        ? 'bg-background text-foreground shadow-sm'
                        : 'text-muted-foreground hover:text-foreground'
                    }`}
                  >
